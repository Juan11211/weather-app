...
function HomeScreen(){
    ...
    return (
        ...
        <h3>{ data.main ? <p className='p'>Humidity: {data.main.humidity} </p>: null}</h3>
        <h3>{ data.main ? <p>Atmospheric pressure: {data.main.pressure} </p>: null}</h3>
        <h3>{ data.main ? <p>High Temp: {data.main.temp_max.toFixed()} </p>: null}</h3>
        <h3>{ data.main ? <p>Low Temp: {data.main.temp_min.toFixed()} </p>: null}</h3>
        </div>
        </div>
    )
}
